...
float mapTerrain( vec3 p )
{
	...
	float f;
    f  = 0.5000*noise3( p*1.00 + vec3(0.0,1.0,0.0)*spe*time );
    f += 0.2500*noise3( p*2.02 + vec3(0.0,2.0,0.0)*spe*time );
    f += 0.1250*noise3( p*4.01 );
	return 25.0*f-10.0;
}
