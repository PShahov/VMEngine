...
vec4 mapStatic(vec3 p){
    ...
    float bw1 = pNoise(p.xz / 10, 0);
    float bw2 = pNoise((p.xz + vec2(100,100)) / 2, 0);
    float bw3 = pNoise((p.xz + vec2(100,100)) / 5, 0);
    bw1 = (bw1 * 2 + bw2 + bw3 * -10) / 2;
    
    //plane
    float planeDist = fPlane(p, vec3(0,1,0), bw1);
    float planeId = 10.0;
...
vec4 mapDynamic(vec3 p){
    ...
    // for(int i = 0;i < u_objects.length;i += 13){
    //     switch(int(u_objects[i])){
    //         case 0:{
    ...
    //         case 1:{
    //             vec4 obj = _model(
    //                 p1 + vec3(u_objects[3], u_objects[4], u_objects[5]),
    //                 int(u_objects[2])
    //                 );
    //             break;
    //         }
    ...
    // }
    // res = 
    // res = fOpUnionRoundId(_model(p1 + vec3(0,0, 3.5), 4), res, 0.5, 1, 0.75);
    // vec3 p2 = p;
    // p2 += vec3(0, -1.255 + (sin(u_alive * 5) * 0.005), -1.75 + (cos(u_alive) * 0.02) );
    // res = fOpUnionId(_model(p2, 5), res);
    // return _model(p2, 5);
    return res;
}
...
vec4 map(vec3 p){
    ...
    // return (fOpUnionRoundId(mapStatic(p), mapDynamic(p), 0.5,));
    // vec4 res;
    ...
    // for(int i = 1;i < 30;i++){
    //     res = fOpUnionId(res, _model(p + vec3((i * 5), 0, 0), 3));
    // }
    // res = fOpUnionId(res, plane);
    // return res;
}
